...
/**
 * Search for a lid-mapping file in SESSION_DIR and its subdirectories.
 * LID mapping files are stored inside session subdirectories like sessions/dina_XXXX/lid-mapping-*.json
 */
function findLidMappingFile(filename) {
    // First try direct SESSION_DIR path (legacy support)
    const directPath = path.join(SESSION_DIR, filename);
    if (fs.existsSync(directPath)) return directPath;

    // Search in each session subdirectory
    try {
        const entries = fs.readdirSync(SESSION_DIR, { withFileTypes: true });
        for (const entry of entries) {
            if (entry.isDirectory()) {
                const subPath = path.join(SESSION_DIR, entry.name, filename);
...
function resolvePhoneFromLid(lidNumber) {
    ...
    try {
        const reverseFile = findLidMappingFile(`lid-mapping-${lidNumber}_reverse.json`);
        if (reverseFile) {
...
function resolveLidFromPhone(phoneNumber) {
    ...
    try {
        const mapFile = findLidMappingFile(`lid-mapping-${phoneNumber}.json`);
        if (mapFile) {
